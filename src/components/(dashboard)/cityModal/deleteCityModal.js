...
import { deleteCity } from "@/fetchData/city";
...
const DeleteCityModal = ({
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      const response = await deleteCity({ id: selectedRowData._id });
      setLoading(false);
      setError(false);
      setSuccessDelete(response);
      setAlertData({
        type: "success",
        message: `City ${selectedRowData.city} has been deleted successfuly `,
      });
      setOpenNote(true);
      handleClose();
    } catch (error) {
      ...
      >
        <Box className={styles.modal}>
          ...
          <Typography variant="p" component="p" mb="1rem">
            ...
            {selectedRowData && selectedRowData.city} ??
          </Typography>
          {loading ? (
